...
run('draw(nodi);');
ins('ff.type==="normal"','while');       // while(1)
ins('ff.type==="loop_body"','if');       // if(2) nel corpo
ins('ff.type==="if_true"&&ff.fromNodeIndex===2','home'); // Casa(home) nel ramo true dell'if
run('draw(nodi);');
...
// transizione Casa(3)->while(1): cattura per fase
...
console.log('\nCasa(3)->while(1) branch=true:');
console.log(' OUT :', JSON.stringify(cap(3,1,'true','out')));
console.log(' BACK:', JSON.stringify(cap(3,1,'true','back')));
