...
run('draw(nodi);'); ins('ff.type==="normal"','for'); ins('ff.type==="loop_body"','if'); ins('ff.type==="if_true"&&ff.fromNodeIndex===2','while'); ins('ff.type==="loop_body"&&ff.fromNodeIndex===3','assign');
...
for(const [f,t,b,lab] of [[2,1,'false','if(2).false -> for(1)'],[3,1,'false','while(3).false -> for(1)'],[4,3,'true','assign(4) -> while(3)']]){
  console.log('\n'+lab+'  FULL:',JSON.stringify(cap(f,t,b,null)));
}
