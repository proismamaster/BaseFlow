...
run('nodi.forEach(function(v,i){});');
console.log('NODI:', run('JSON.stringify(flow.nodes.map(function(n,i){return i+":"+n.type;}))'));
// spia highlight
run('globalThis.__seq=[]; var _hn=highlightExecNode; highlightExecNode=function(i){__seq.push("NODO("+i+")"); return _hn.apply(this,arguments);}; var _he=highlightExecEdge; highlightExecEdge=function(f,t,p){__seq.push("ARCO("+f+"->"+t+(p?","+p:"")+")"); return _he.apply(this,arguments);};');
// simula "pausa DURANTE la fase-arco tra print1(1) e print2(2)": A gia' eseguito, currentNode=2 (B pendente), arco 1->2 acceso
...
console.log('stato pausa-su-arco: currentNode=2 (B pendente), executingEdge=', run('JSON.stringify(executingEdge)'));
(async()=>{
 ...
 console.log('sequenza highlight premendo PASSO da pausa-su-arco:', run('JSON.stringify(__seq)'));
 console.log('  -> executingEdge dopo:', run('JSON.stringify(executingEdge)'), ' currentNode dopo:', run('String(currentNode)'));
})();
