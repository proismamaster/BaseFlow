...
// WP-M5y: rettangolo VISIBILE in coordinate LOGICHE del canvas (le stesse di relX*w / relY*h).
// Ritorna null quando non e' determinabile -- in quel caso non si culla nulla: meglio disegnare
// di piu' che rischiare un blocco invisibile. Il culling si disattiva anche durante l'EXPORT
// (immagine/PDF), dove il "visibile" non ha senso: li' serve il diagramma intero.
var _bfExporting = false; // true SOLO durante la generazione di un'immagine/PDF (vedi sotto)
function _bfViewportBox() {
  if (typeof document === 'undefined' || !document.getElementById) return null;
  if (_bfExporting) return null;
  const cont = document.getElementById('canvas-container');
  if (!cont || !cont.clientWidth || !cont.clientHeight) return null;
  const z = (typeof zoom === 'number' && zoom > 0) ? zoom : 1;
  let cvLeft = 0, cvTop = 0;
  try { if (typeof canvas !== 'undefined' && canvas) { cvLeft = canvas.offsetLeft || 0; cvTop = canvas.offsetTop || 0; } } catch (e) {}
  const m = 300; // margine: nessun blocco "appare" mentre si scorre
  return {
    x1: (cont.scrollLeft - cvLeft) / z - m,
    y1: (cont.scrollTop - cvTop) / z - m,
    x2: (cont.scrollLeft - cvLeft + cont.clientWidth) / z + m,
    y2: (cont.scrollTop - cvTop + cont.clientHeight) / z + m
  };
}
function draw(forme) {
  ...
  // WP-M5v: la griglia NON si disegna piu' qui. Dipinta sul canvas copriva solo il canvas, e
  // il canvas non copre sempre il contenitore (pannelli chiusi, zoom sotto il 100%): restavano
  // fasce senza griglia. Ora e' lo sfondo CSS di #canvas-container -- vedi _bfSyncCanvasGrid in
  // layout.js. Vantaggio secondario: non costa piu' nulla ad ogni draw() (erano ~2 x centinaia
  // di segmenti per frame, ridisegnati ad ogni passo dell'esecuzione).
  ...
  // WP-M5y (Ismail 2026-07-21, ispirato al "flowchart culling" di Flogorithm): non si
  // DIPINGONO i blocchi che stanno fuori dalla parte visibile. Su un diagramma grande, dove
  // a schermo ce ne stanno una decina su cento, e' quasi tutto lavoro buttato -- e si ripete a
  // ogni ridisegno, quindi molte volte al secondo durante l'esecuzione.
  // Importante: si salta solo la PITTURA. La geometria (relX/relY, larghezze, archi, elenco
  // `frecce` per il rilevamento dei click) e' calcolata prima e resta intatta: un blocco fuori
  // schermo si puo' ancora selezionare, spostare, collegare. Cullare anche quella
  // introdurrebbe bug sottili in cambio di nulla, perche' costa poco.
  // Il margine di 300px evita che un blocco a cavallo del bordo sparisca durante lo scroll.
  const _cull = _bfViewportBox();
  for (let i = 0; i < forme.length; i++) {
    const node = forme[i]; // Nodo visuale corrente
    if (!node) continue;
    if (_cull) {
      const _nx = node.relX * w, _ny = node.relY * h;
      const _hw = (node.width || 100) / 2, _hh = (node.height || 40) / 2;
      if (_nx + _hw < _cull.x1 || _nx - _hw > _cull.x2 || _ny + _hh < _cull.y1 || _ny - _hh > _cull.y2) continue;
    }
    ...
    if (_multiSelSet.has(i)) {
      ctx.save();
      ctx.setLineDash([6, 4]);
      // 2026-07-19 (Ismail): "marching ants" -- durante il drag di gruppo i trattini SCORRONO
      // (lineDashOffset animato da _bfAntsOffset, interaction.js). A riposo (selezione ferma,
      // _bfAntsOffset=0) il bordo è statico come prima. Offset negativo = i trattini "marciano"
      // in senso orario lungo il perimetro.
      if (typeof _bfAntsOffset === 'number' && _bfAntsOffset) ctx.lineDashOffset = -_bfAntsOffset;
      ctx.strokeStyle = (typeof cssVar === "function") ? cssVar('--node-selected-color', '#1e88e5') : '#1e88e5';
      ctx.lineWidth = 2.5;
      ctx.stroke();
      ctx.restore();
    ...
    if (isBeingDragged) {
      ctx.save();
      ctx.setLineDash([6, 4]);
      // 2026-07-19: stesso "marching ants" del bordo di selezione multipla sopra -- così, in
      // un drag di gruppo, il perimetro del/i blocco/i trascinato/i "scorre" in modo coeso.
      if (typeof _bfAntsOffset === 'number' && _bfAntsOffset) ctx.lineDashOffset = -_bfAntsOffset;
  ...
  if (typeof isDraggingNode !== "undefined" && isDraggingNode && typeof dragNodeIndex !== "undefined" && dragNodeIndex !== -1 && forme[dragNodeIndex]) {
    // 2026-07-19 (Ismail, "anima solo il blocco che trascini, non tutta la selezione"): in un
    // drag di GRUPPO ora ogni membro della selezione ha il proprio ghost, traslato della stessa
    // distanza cursore↔blocco-afferrato -> l'intera selezione si SOLLEVA e SEGUE il cursore
    // insieme (prima seguiva un solo ghost). L'offset marching ants (_bfAntsOffset) fa scorrere
    // i bordi. Per un drag singolo resta un solo ghost centrato sul cursore (comportamento storico).
    const _grab = forme[dragNodeIndex];
    const _cursorX = (typeof dragCurrentX === "number") ? dragCurrentX : _grab.relX * w;
    const _cursorY = (typeof dragCurrentY === "number") ? dragCurrentY : _grab.relY * h;
    // 2026-07-20 (Ismail, "il cursore prenda il blocco dalla CIMA, non dal mezzo, così vedi
    // dove lo stai trascinando"): il blocco afferrato "pende" SOTTO il cursore -- il suo bordo
    // superiore sta al cursore (+ un piccolo gap), non il suo centro. L'ancora verticale a cui
    // convergono/si posizionano i ghost è quindi cursore + mezzaAltezza + gap. Il drop resta al
    // cursore (arcHitTest usa le coord del puntatore, indipendenti dal ghost): puramente visivo.
    const _grabHalfH = (((_grab && _grab.height) || NODE_BASE_HEIGHT_PX) / 2);
    const _GRAB_TOP_GAP = 6;
    const _anchorX = _cursorX;
    const _anchorY = _cursorY + _grabHalfH + _GRAB_TOP_GAP;
    const _ddx = _anchorX - _grab.relX * w;
    const _ddy = _anchorY - _grab.relY * h;
    const _ants = (typeof _bfAntsOffset === "number" && _bfAntsOffset) ? -_bfAntsOffset : 0;
    let _members = null;
    if (typeof dragScattered !== "undefined" && dragScattered && _multiSelSet && _multiSelSet.size > 1) {
      _members = Array.from(_multiSelSet);
    } else if (typeof dragSubtreeEnd !== "undefined" && dragSubtreeEnd > dragNodeIndex + 1) {
      // 2026-07-20 (Ismail, "si deve fare anche se trascini un nodo padre, i figli fanno lo
      // stesso"): copre SIA il drag di gruppo contiguo SIA il drag di un SINGOLO blocco padre
      // (IF/ciclo) col suo sottoalbero -- in entrambi i casi dragSubtreeEnd va oltre il blocco
      // afferrato, quindi il range [dragNodeIndex, dragSubtreeEnd) sono tutti i blocchi che si
      // muovono: si raccolgono nella pila col badge del conteggio come per una selezione multipla.
      _members = []; for (let _m = dragNodeIndex; _m < dragSubtreeEnd; _m++) _members.push(_m);
    }
    if (_members && _members.length > 1) {
      // 2026-07-20 (Ismail): animazione di RACCOLTA. All'inizio del drag i ghost partono dalle
      // loro posizioni reali (traslate col cursore) e CONVERGONO sul blocco afferrato (sotto il
      // cursore) in ~170ms; poi restano impilati lì con un BADGE che mostra il conteggio.
      const _nowT = (typeof performance !== "undefined" && performance.now) ? performance.now() : Date.now();
      const _COLLAPSE_MS = 290; // 2026-07-20: raccolta un po' più lenta (era 170, "troppo veloce" per Ismail)
      let _p = (typeof _bfDragCollapseT0 === "number" && _bfDragCollapseT0) ? Math.min(1, (_nowT - _bfDragCollapseT0) / _COLLAPSE_MS) : 1;
      const _e = 1 - Math.pow(1 - _p, 3); // easeOutCubic: parte veloce, rallenta alla fine
      // il blocco AFFERRATO va disegnato per ULTIMO (in cima alla pila, sotto il cursore).
      _members.sort(function (a, b) { return (a === dragNodeIndex ? 1 : 0) - (b === dragNodeIndex ? 1 : 0); });
      for (const _m of _members) {
        if (!forme[_m] || !flow.nodes[_m]) continue;
        const _sx = forme[_m].relX * w + _ddx, _sy = forme[_m].relY * h + _ddy; // posizione "sparsa" (segue il cursore)
        const _gx = _sx + (_anchorX - _sx) * _e; // interpola verso l'ancora (blocco afferrato) man mano che _e -> 1
        const _gy = _sy + (_anchorY - _sy) * _e;
        drawDragGhost(forme[_m], _m, _gx, _gy, _ants);
      }
      // BADGE col numero di blocchi trascinati, in alto a destra della pila (all'ancora, cioè
      // in cima al blocco afferrato ~al cursore); sfuma in mentre i blocchi si raccolgono (_e -> 1).
      if (typeof _bfDrawDragCountBadge === "function") _bfDrawDragCountBadge(_anchorX, _anchorY, _grab, _members.length, _e);
    } else {
      // Drag di un singolo blocco semplice: anch'esso preso dalla CIMA (pende sotto il cursore).
      drawDragGhost(forme[dragNodeIndex], dragNodeIndex, _anchorX, _anchorY, _ants);
    }
  }
}
// 2026-07-20: badge circolare col NUMERO di blocchi trascinati (drag di gruppo), disegnato in
// alto a destra della pila di ghost. alpha 0->1 mentre i blocchi si raccolgono, così "appare"
// insieme alla pila. Colori: cerchio = colore di feedback-drag (arcDragColor), numero bianco.
function _bfDrawDragCountBadge(cx, cy, grabNode, n, alpha) {
  if (!ctx || !(n > 1)) return;
  const bw = (grabNode && grabNode.width) ? grabNode.width : 100;
  const bh = (grabNode && grabNode.height) ? grabNode.height : 40;
  const r = 13;
  const px = cx + bw / 2 - 2;   // angolo alto-destra del blocco sotto il cursore
  const py = cy - bh / 2 + 2;
  ctx.save();
  ...
  ctx.closePath();
  ctx.fillStyle = (typeof arcDragColor === "function") ? arcDragColor() : "#1e88e5";
  ...
  ctx.stroke();
  ctx.fillStyle = "#fff";
  ctx.font = "bold " + (n > 99 ? 12 : 14) + "px Arial";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(n > 99 ? "99+" : String(n), px, py + 0.5);
  ctx.restore();
}

// Disegna il "ghost" di un nodo trascinato. Di default centrato sul cursore (dragCurrentX/Y,
// drag di singolo blocco); con cxOverride/cyOverride si posiziona a coordinate arbitrarie --
// usato dal drag di GRUPPO, dove OGNI membro della selezione ha il proprio ghost traslato
// della stessa distanza cursore↔blocco-afferrato (2026-07-19: prima seguiva il cursore un
// SOLO ghost, "animava solo il blocco che trascini"; ora tutta la selezione si solleva e
// segue insieme). dashOffset != 0 fa "marciare" il tratteggio del bordo (marching ants).
function drawDragGhost(node, nodeIndex, cxOverride, cyOverride, dashOffset) {
  ...
  const gx = (typeof cxOverride === "number") ? cxOverride : (typeof dragCurrentX === "number" ? dragCurrentX : node.relX * w);
  const gy = (typeof cyOverride === "number") ? cyOverride : (typeof dragCurrentY === "number" ? dragCurrentY : node.relY * h);
  ...
  ctx.lineWidth = 2.5;
  ctx.setLineDash([6, 4]);
  if (dashOffset) ctx.lineDashOffset = dashOffset; // marching ants sul ghost di gruppo
  ctx.shadowColor = "rgba(0,0,0,0.35)";
  ctx.shadowBlur = 10;
...
function drawLoopBranches(loopIdx, node) {
  ...
  // WP-M5e (Ismail 2026-07-21, "in italiano Prossimo viene sovrapposto"): l'etichetta era
  // sempre centrata a meta' del tratto fra l'esagono e la colonna del corpo. Con "Next"
  // (4 caratteri) il mezzo tratto bastava; con "Prossimo" -- e ancor piu' con le lingue in
  // cui la parola e' lunga -- la meta' sinistra del testo finiva SOPRA la punta destra
  // dell'esagono. Il punto centrale non e' quindi una posizione valida in generale: si
  // misura il testo e, se il suo bordo sinistro entrerebbe nell'esagono, lo si sposta a
  // destra quanto basta. Con etichette corte il risultato e' identico a prima (la
  // condizione non scatta), quindi nessuna regressione sui flow gia' visti in inglese.
  {
    const _bodyLbl = (typeof edgeLabelText === 'function') ? edgeLabelText(loopType, 'body') : labels.body;
    const _w = ctx.measureText(_bodyLbl).width;
    const _minLeft = diaRight + 8; // 8px d'aria fra la punta dell'esagono e la parola
    let _x = (diaRight + bodyColX) / 2;
    if (_x - _w / 2 < _minLeft) _x = _minLeft + _w / 2;
    ctx.fillText(_bodyLbl, _x, cy - 10);
  }
  ctx.fillStyle = cssVar('--if-false-color', IF_LABEL_FALSE_COLOR);
