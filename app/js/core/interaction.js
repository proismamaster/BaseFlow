...
// 2026-07-19 (Ismail): "marching ants" durante il drag di gruppo. Loop rAF che anima
// _bfAntsOffset (letto da rendering come ctx.lineDashOffset) cosi' i trattini del bordo di
// selezione SCORRONO anche a puntatore FERMO, non solo quando il mouse/dito si muove (il
// pointermove ridisegna gia', ma si ferma appena il gesto è immobile). Si AUTO-FERMA appena
// il drag di gruppo finisce: nessun rAF perenne, nessun costo a riposo (rispetto batteria/CPU
// mobile). Per un drag di SINGOLO nodo la condizione è falsa -> un tick a vuoto e stop.
function _bfGroupDragAnimActive() {
  return (typeof isDraggingNode !== 'undefined' && isDraggingNode) && (
    (typeof dragIsGroup !== 'undefined' && dragIsGroup) ||
    (typeof dragScattered !== 'undefined' && dragScattered) ||
    (typeof multiSelected !== 'undefined' && multiSelected && multiSelected.length > 1) ||
    // 2026-07-20: anche il drag di un SINGOLO blocco padre (IF/ciclo) col suo sottoalbero
    // (dragSubtreeEnd oltre il blocco afferrato) anima raccolta + marching ants.
    (typeof dragSubtreeEnd !== 'undefined' && typeof dragNodeIndex !== 'undefined' && dragSubtreeEnd > dragNodeIndex + 1)
  );
}
function _bfGroupDragAnimTick() {
  _bfAntsRAF = null;
  if (!_bfGroupDragAnimActive()) { _bfAntsOffset = 0; return; } // stop pulito
  // ~30px/s a 60fps: abbastanza vivo da notarsi, abbastanza lento da non "sfarfallare".
  _bfAntsOffset = (_bfAntsOffset + 0.6) % 10000;
  if (typeof draw === 'function' && typeof nodi !== 'undefined') draw(nodi);
  const raf = (typeof window !== 'undefined' && window.requestAnimationFrame) ? window.requestAnimationFrame : function (f) { return setTimeout(f, 16); };
  _bfAntsRAF = raf(_bfGroupDragAnimTick);
}
function _bfStartGroupDragAnim() {
  if (_bfAntsRAF != null) return; // già in corso
  const raf = (typeof window !== 'undefined' && window.requestAnimationFrame) ? window.requestAnimationFrame : function (f) { return setTimeout(f, 16); };
  _bfAntsRAF = raf(_bfGroupDragAnimTick);
}
...
function onCanvasMouseMove(event) {
  ...
  if (!isDraggingNode) {
    ...
    isDraggingNode = true;
    // 2026-07-19: appena il drag supera la soglia, avvia il "marching ants" (il tick si
    // auto-ferma subito se NON è un drag di gruppo -> nessun effetto per il singolo nodo).
    _bfStartGroupDragAnim();
    // 2026-07-20: avvia l'animazione di "raccolta" (i ghost convergono verso il blocco
    // afferrato) per un drag di GRUPPO **o** di un singolo PADRE col suo sottoalbero (IF/ciclo:
    // dragSubtreeEnd oltre il blocco afferrato). Timestamp d'inizio letto da rendering.
    if ((typeof dragIsGroup !== 'undefined' && dragIsGroup) || (typeof dragScattered !== 'undefined' && dragScattered) ||
        (typeof dragSubtreeEnd !== 'undefined' && dragSubtreeEnd > dragNodeIndex + 1)) {
      _bfDragCollapseT0 = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
    } else {
      _bfDragCollapseT0 = 0;
    }
  }
...
function onCanvasMouseUp(event) {
  ...
  _bfAntsOffset = 0; // 2026-07-19: ferma il "marching ants" e azzera l'offset per il frame finale pulito
  _bfDragCollapseT0 = 0; // 2026-07-20: azzera l'animazione di raccolta
...
function onCanvasPointerCancel(event) {
  ...
  _bfAntsOffset = 0; // 2026-07-19: ferma il "marching ants"
  _bfDragCollapseT0 = 0; // 2026-07-20: azzera l'animazione di raccolta
  if (canvas && canvas.style) canvas.style.cursor = 'default';
  if (typeof draw === 'function' && typeof nodi !== 'undefined') draw(nodi);
}
...
// WP-M (Ismail 2026-07-20): timestamp dell'ultimo tocco concluso sul canvas -- clickNodo lo
// usa per riconoscere il click SINTETICO generato da un tap (quando event.pointerType non
// e' disponibile sull'evento click) e attivare il comportamento touch "tap = aggiungi alla
// selezione" (vedi clickNodo).
let _bfLastTouchTs = 0;
function _bfIsTouchLikeEvent(event) {
  if (event && event.pointerType) return event.pointerType === 'touch';
  return (Date.now() - _bfLastTouchTs) < 700;
}
function onCanvasTouchPointerDown(event) {
  ...
  _touchLongPressTimer = setTimeout(function () {
    ...
    // WP-M (Ismail 2026-07-20): long-press SU UN BLOCCO = selezione (entra/esce dalla
    // selezione multipla, stesso toggle del Ctrl+click desktop). Il long-press su area
    // VUOTA o su un arco resta il menu contestuale di prima (serve ancora per Incolla,
    // e sul blocco le stesse azioni restano nel menu contestuale del gruppo via
    // long-press su area vuota o pulsanti UI). Cosi' da telefono: tieni premuto un
    // blocco -> si seleziona; poi ogni TAP su un altro blocco lo aggiunge (clickNodo).
    const _pt = { clientX: cx, clientY: cy };
    let _hitIdx = -1;
    try {
      const _cc = canvasCoordsFromEvent(_pt);
      _hitIdx = hitTestNode(_cc.x, _cc.y);
    } catch (e) { _hitIdx = -1; }
    if (_hitIdx !== -1 && flow.nodes[_hitIdx] && flow.nodes[_hitIdx].type !== 'start' && flow.nodes[_hitIdx].type !== 'end') {
      // Il long-press avviene A META' di un pointerdown gia' partito: il drag&drop ha gia'
      // "afferrato" il nodo (onCanvasMouseDown). La selezione qui NON deve annullare quel
      // drag: se l'utente continua a muovere il dito, trascina il nodo (o il gruppo appena
      // esteso); se solleva il dito, resta la selezione.
      if (typeof toggleMultiSelect === 'function') toggleMultiSelect(_hitIdx);
...
function onCanvasTouchPointerUp(event) {
  if (event.pointerType !== 'touch') return;
  _bfLastTouchTs = Date.now();
  _cancelTouchLongPress();
}

// ============================================================================
// WP-M (Ismail 2026-07-20): PINCH-TO-ZOOM a 2 dita sul canvas -- zooma il GRAFO
// (variabile `zoom`, stessi limiti ZOOM_MIN/MAX dei pulsanti), MAI la pagina intera.
// Listener registrati in init.js. Con 2 dita attive qualunque drag/long-press in corso
// viene annullato (il gesto e' inequivocabilmente uno zoom, non uno spostamento).
// ============================================================================
const _bfPinchPts = new Map(); // pointerId -> {x, y}
let _bfPinchActive = false;
let _bfPinchD0 = 0, _bfPinchZ0 = 1;
let _bfPinchRafPending = false;
function _bfPinchDist() {
  const pts = Array.from(_bfPinchPts.values());
  if (pts.length < 2) return 0;
  return Math.hypot(pts[1].x - pts[0].x, pts[1].y - pts[0].y);
}
function onCanvasPinchPointerDown(event) {
  if (event.pointerType !== 'touch') return;
  _bfPinchPts.set(event.pointerId, { x: event.clientX, y: event.clientY });
  if (_bfPinchPts.size === 2) {
    _bfPinchActive = true;
    _bfPinchD0 = _bfPinchDist();
    _bfPinchZ0 = (typeof zoom !== 'undefined') ? zoom : 1;
    // Annulla long-press e drag in corso: seconda falange = zoom, non spostamento.
    if (typeof _cancelTouchLongPress === 'function') _cancelTouchLongPress();
    if (typeof onCanvasPointerCancel === 'function') { try { onCanvasPointerCancel(event); } catch (e) {} }
    suppressNextClick = true;
  }
}
function onCanvasPinchPointerMove(event) {
  if (event.pointerType !== 'touch' || !_bfPinchPts.has(event.pointerId)) return;
  _bfPinchPts.set(event.pointerId, { x: event.clientX, y: event.clientY });
  if (!_bfPinchActive || _bfPinchPts.size < 2 || !_bfPinchD0) return;
  const d = _bfPinchDist();
  if (!d) return;
  let nz = _bfPinchZ0 * (d / _bfPinchD0);
  nz = Math.max((typeof ZOOM_MIN !== 'undefined') ? ZOOM_MIN : 0.15, Math.min((typeof ZOOM_MAX !== 'undefined') ? ZOOM_MAX : 4, nz));
  if (typeof zoom !== 'undefined' && Math.abs(nz - zoom) / (zoom || 1) > 0.01 && !_bfPinchRafPending) {
    _bfPinchRafPending = true;
    (window.requestAnimationFrame || function (fn) { return setTimeout(fn, 16); })(function () {
      _bfPinchRafPending = false;
      zoom = +nz.toFixed(4);
      if (typeof _bfZoomRerender === 'function') _bfZoomRerender();
      if (typeof _bfLayoutTickOrCenter === 'function') _bfLayoutTickOrCenter();
    });
  }
}
function onCanvasPinchPointerUp(event) {
  if (event.pointerType && event.pointerType !== 'touch') return;
  _bfPinchPts.delete(event.pointerId);
  if (_bfPinchPts.size < 2) { _bfPinchActive = false; _bfPinchD0 = 0; }
}
  ...
  function clickNodo(event) {
    ...
    for (let i = 0; i < nodi.length; i++) {
      ...
      if (clickX >= x0 && clickX <= x1 && clickY >= y0 && clickY <= y1) {
        ...
        // WP-M (Ismail 2026-07-20): da TOUCH, con una selezione multipla gia' attiva
        // (avviata col long-press su un blocco), ogni TAP su un altro blocco lo
        // AGGIUNGE/RIMUOVE dalla selezione -- equivalente a dito del Ctrl+click desktop
        // (che su telefono non esiste). Il tap su area vuota (clickFreccia) o Esc
        // azzerano la selezione come sempre.
        if (_bfIsTouchLikeEvent(event) && typeof multiSelected !== 'undefined' && multiSelected.length) {
          toggleMultiSelect(i);
          return;
        }
