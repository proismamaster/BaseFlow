...
function ensureTurtlePanel() {
  ...
  // WP-M (Ismail 2026-07-20): PINCH a 2 dita sul pannello tartaruga = zoom della TELA
  // (_tgZoom, stessi limiti dei pulsanti +/-), non della pagina. preventDefault sul
  // touchmove a 2 dita blocca il pinch-zoom nativo del browser solo qui dentro.
  (function () {
    let d0 = 0, z0 = 1;
    ...
    p.addEventListener('touchstart', function (e) {
      if (e.touches.length === 2) { d0 = dist(e); z0 = _tgZoom; }
    }, { passive: true });
    p.addEventListener('touchmove', function (e) {
      if (e.touches.length !== 2 || !d0) return;
      e.preventDefault();
      let nz = z0 * (dist(e) / d0);
      nz = Math.max(TG_ZOOM_MIN, Math.min(TG_ZOOM_MAX, nz));
      if (Math.abs(nz - _tgZoom) / (_tgZoom || 1) > 0.01) { _tgZoom = +nz.toFixed(3); _tgApplyZoom(); }
    }, { passive: false });
    const end = function (e) { if (!e.touches || e.touches.length < 2) d0 = 0; };
...
// WP-M6a (Ismail 2026-07-21, "se ci sono le linee allora devono essere coerenti anche nella
// tartaruga"): stessa griglia del canvas principale, stessa impostazione (viewSettings.showGrid),
// stesso passo e stesso colore -- altrimenti l'app avrebbe due aree di disegno con due regole
// diverse, e l'impostazione si chiamerebbe "griglia" mentre ne griglia solo una.
// Differenza obbligata rispetto al canvas principale: li' lo sfondo sta sul CONTENITORE, qui
// deve stare sul CANVAS. Il motivo e' che la tela della tartaruga ha uno sfondo BIANCO pieno
// (`#draw-canvas { background: #fff }`): una griglia messa sul contenitore finirebbe sotto quel
// bianco e non si vedrebbe. Sul canvas invece il bianco diventa il fondo della griglia stessa,
// e il disegno -- che sta nel bitmap, trasparente dove non si e' disegnato -- ci passa sopra.
// L'ancoraggio e' la CASA della tartaruga (origine 0,0): le linee si incrociano li', quindi la
// griglia dice qualcosa di utile sulle coordinate invece di essere un motivo decorativo.
function _bfSyncTurtleGrid() {
  ...
  if (!on) { cv.style.backgroundImage = ''; return; }
  const step = 24 * _tgZoom; // stesso passo logico del canvas principale (BF_GRID_STEP)
  const base = (typeof themeCanvasLineColor === 'function') ? themeCanvasLineColor() : '#000';
  const col = 'color-mix(in srgb, ' + base + ' 8%, transparent)';
  const ox = _tgOriginX * _tgZoom, oy = _tgOriginY * _tgZoom;
  ...
  cv.style.backgroundSize = step + 'px ' + step + 'px, ' + step + 'px ' + step + 'px';
  cv.style.backgroundPosition = ox + 'px ' + oy + 'px, ' + ox + 'px ' + oy + 'px';
}
if (typeof window !== 'undefined') window._bfSyncTurtleGrid = _bfSyncTurtleGrid;
...
// WP-M5x (Ismail 2026-07-21, confronto con le impostazioni "Advanced" di Flogorithm): la
// tartaruga aveva un costo QUADRATICO nel numero di mosse, e nessuno l'aveva mai segnalato
// perche' si manifesta solo sui disegni lunghi. Ad OGNI mossa si faceva:
//   1. _tgRecomputeBounds(): scansione di TUTTI i segmenti per ricalcolare la bounding box;
//   2. _tgApplyZoom() -> _tgResizeCanvases() -> _tgRenderDrawing(): ridisegno di TUTTI i
//      segmenti da capo su una tela ripulita.
// Due passate O(N) per mossa = O(N²) sul programma: 5000 mosse ~ 12 milioni di operazioni.
// Correzione in due parti, entrambe basate su un'osservazione semplice -- una mossa aggiunge
// UN segmento, e quasi sempre NON cambia la dimensione della tela:
//   (a) i limiti si tengono aggiornati in modo INCREMENTALE (_tgB), invece di riscoprirli
//       ogni volta scandendo la storia; la scansione completa resta disponibile per i casi in
//       cui i segmenti cambiano in blocco (reset, clear, caricamento di un file);
//   (b) se la geometria della tela non cambia, si disegna SOLO il segmento nuovo sopra quello
//       che c'e' gia'; il ridisegno completo serve solo quando la tela cresce davvero, perche'
//       li' cambiano origine e dimensioni e tutto va riproiettato.
var _tgB = { minX: 0, maxX: 0, minY: 0, maxY: 0 }; // limiti correnti (casa 0,0 sempre inclusa)
function _tgBoundsInclude(x, y) {
  if (x < _tgB.minX) _tgB.minX = x; if (x > _tgB.maxX) _tgB.maxX = x;
  if (y < _tgB.minY) _tgB.minY = y; if (y > _tgB.maxY) _tgB.maxY = y;
}
// Riscansione COMPLETA: solo quando _tgSegments cambia in modo non incrementale.
function _tgRecomputeBoundsFull() {
  _tgB = { minX: 0, maxX: 0, minY: 0, maxY: 0 };
  for (let i = 0; i < _tgSegments.length; i++) {
    const sg = _tgSegments[i];
    _tgBoundsInclude(sg.x1, sg.y1);
    _tgBoundsInclude(sg.x2, sg.y2);
  }
  return _tgApplyBounds();
}
// Traduce i limiti correnti in dimensione della tela + offset. Ritorna true se la GEOMETRIA
// e' cambiata: e' il segnale che serve un ridisegno completo (origine e dimensioni diverse).
function _tgApplyBounds() {
  _tgBoundsInclude(_tg.x, _tg.y); // la tartaruga stessa fa parte dei limiti
  // WP-M5x: geometria ARROTONDATA a interi. Non e' cosmetica: la posizione della tartaruga si
  // accumula per somme di seni e coseni, quindi ridisegnando piu' volte la stessa figura i
  // limiti oscillano di frazioni infinitesime (es. 320.00000000000176 invece di 320). Con un
  // confronto fra float quelle oscillazioni risultavano "tela cambiata" e facevano scattare un
  // ridisegno COMPLETO ogni pochi passi -- misurato: 64 ridisegni inutili su 2000 mosse.
  // Le dimensioni di una tela sono comunque pixel interi: arrotondando, una figura ripetuta
  // produce sempre gli stessi numeri e il ridisegno completo scatta solo quando cresce davvero.
  const w = Math.round(Math.max(300, (_tgB.maxX - _tgB.minX) + TG_MARGIN * 2));
  const h = Math.round(Math.max(300, (_tgB.maxY - _tgB.minY) + TG_MARGIN * 2));
  const ox = Math.round(TG_MARGIN - _tgB.minX);
  const oy = Math.round(TG_MARGIN - _tgB.minY);
  const changed = (w !== TG_W || h !== TG_H || ox !== _tgOriginX || oy !== _tgOriginY);
  TG_W = w; TG_H = h; _tgOriginX = ox; _tgOriginY = oy;
  return changed;
}
// Compat: il nome storico resta, ma ora costa O(1) invece di O(N).
function _tgRecomputeBounds() { return _tgApplyBounds(); }
// Disegna UN solo segmento sulla tela esistente (nessun clear, nessuna riproiezione).
function _tgStrokeSegment(sg) {
  const g = _tgApplyTransform('draw-canvas'); if (!g || !sg) return;
  g.strokeStyle = sg.color; g.lineWidth = Math.max(0.4, sg.width); g.lineCap = 'round';
  g.beginPath();
  g.moveTo(sg.x1 + _tgOriginX, sg.y1 + _tgOriginY);
  g.lineTo(sg.x2 + _tgOriginX, sg.y2 + _tgOriginY);
  g.stroke();
}
...
var _tgZoom = 1;
// WP-M (Ismail 2026-07-20): range ampliato (era 0.4..5) su richiesta "aumentare zoom/unzoom max".
var TG_ZOOM_MIN = 0.15, TG_ZOOM_MAX = 10;
function tgZoomIn() { _tgZoom = Math.min(TG_ZOOM_MAX, +(_tgZoom * 1.2).toFixed(3)); _tgApplyZoom(); }
function tgZoomOut() { _tgZoom = Math.max(TG_ZOOM_MIN, +(_tgZoom / 1.2).toFixed(3)); _tgApplyZoom(); }
...
// WP-M5x: `_tgApplyZoom()` (che ridimensiona le tele e RIDISEGNA tutto il disegno da capo)
// veniva eseguito ad OGNI chiamata -- e questa funzione la chiamano execForward, execTurn,
// execHome e execClearScreen, cioe' praticamente ogni istruzione grafica. Era il vero costo
// quadratico della tartaruga: piu' del ricalcolo dei limiti, che pure era O(N).
// Il ridimensionamento serve solo quando il pannello passa da NASCOSTO a visibile (prima di
// allora le tele non hanno dimensioni utili); a pannello gia' aperto lo fanno gia', quando
// serve davvero, i cambi di zoom e le crescite della tela.
function showTurtlePanel() {
  const p = ensureTurtlePanel();
  if (!p) return p;
  const wasHidden = (p.style.display !== 'flex');
  p.style.display = 'flex';
  if (wasHidden) _tgApplyZoom();
  if (typeof bfBringToFront === 'function') bfBringToFront(p);
  return p;
}
...
function resetTurtle() {
  ...
  _tgSegments = [];
  _tgRecomputeBoundsFull(); // WP-M5x: reset totale -> riscansione (torna a 300x300)
  _tgRenderDrawing();
  redrawTurtleMarker();
}
...
function _tgEval(expr, variables, currentNode) {
  ...
  if (e === '') return 0;
  // WP-M5 (audit array 2026-07-21): la sostituzione usava `v.value.toString()`, che per un
  // ARRAY produce "1,2,3" (non un letterale valido: `a[0]` diventava `1,2,3[0]`) e per una
  // STRINGA produce testo NUDO non quotato (bug pre-esistente, stessa famiglia di R13-M
  // gia' corretta in condizioni/Assegna/Output). Ora si usa _varValueForExpr, l'UNICA
  // funzione di serializzazione del progetto: array -> [1,2,3], stringhe quotate+escaped,
  // numeri/bool invariati. Cosi' nei blocchi grafici funzionano `a[i]`, `a.length` e le
  // stringhe, esattamente come in ogni altro campo.
  if (Array.isArray(variables)) {
    // WP-M5: variabile dichiarata ma non inizializzata usata qui -> errore esplicito
    // (stesso contratto di Assegna/Output, WP-N7), invece di valere 0 in silenzio.
    if (typeof _bfUninitUsedIn === 'function') {
      const _uv = _bfUninitUsedIn(e, variables);
      if (_uv) { if (typeof throwError === 'function') throwError(errMsg('err_uninit_var', {n: currentNode, v: _uv})); return NaN; }
    }
    variables.forEach(function (v) {
      const _lit = (typeof _varValueForExpr === 'function') ? _varValueForExpr(v) : String(v.value);
      e = e.replace(new RegExp('\\b' + v.name + '\\b', 'g'), _lit);
    });
  }
  let val;
  try { val = (typeof safeEvaluate === 'function') ? safeEvaluate(e) : parseFloat(e); }
  catch (err) {
    // WP-M5: errori di indicizzazione array (indice fuori range / non intero / non-array)
    // tradotti nel loro messaggio specifico invece del generico "espressione grafica".
    if (typeof throwError === 'function') {
      const _m = (err && err.message) ? String(err.message) : '';
      if (/^__IDX/.test(_m) && typeof _evalErrMsg === 'function') throwError(_evalErrMsg(err, currentNode, 'err_graphics_expr'));
      else throwError(errMsg('err_graphics_expr', {n: currentNode, e: expr}));
    }
    return NaN;
  }
...
function execForward(distExpr, variables, currentNode) {
  ...
  let seg = null;
  if (_tg.pen) { seg = { x1: _tg.x, y1: _tg.y, x2: nx, y2: ny, color: _tg.color, width: _tg.width }; _tgSegments.push(seg); }
  _tg.x = nx; _tg.y = ny;
  // FIX (Ismail 2026-07-10): la tartaruga puo' andare OLTRE la tela attuale -> la bounding
  // box va comunque aggiornata (puo' crescere) e zoom/scroll riallineati.
  // WP-M5x: i limiti si aggiornano ora in modo incrementale (O(1)) e il ridisegno COMPLETO
  // avviene solo se la tela e' davvero cambiata. Nel caso normale -- la tartaruga si muove
  // dentro l'area gia' disegnata -- si traccia il solo segmento nuovo.
  if (seg) { _tgBoundsInclude(seg.x1, seg.y1); _tgBoundsInclude(seg.x2, seg.y2); }
  const _grew = _tgRecomputeBounds();
  if (_grew) { _tgApplyZoom(); }   // _tgApplyZoom -> _tgResizeCanvases -> ridisegno completo
  else { _tgStrokeSegment(seg); redrawTurtleMarker(); }
  _tgScrollToTurtle();
...
function execClearScreen() {
  ...
  _tg.x = TG_HOME_X; _tg.y = TG_HOME_Y; _tg.head = 0;
  showTurtlePanel();
  _tgRecomputeBoundsFull(); _tgApplyZoom(); // WP-M5x: i segmenti spariscono in blocco -> riscansione
