...
// WP-M (Ismail 2026-07-20, "i colori in Crea tema devono essere coerenti al 100% col tema
// attivo, anche per i temi gia' esistenti"): alcuni temi builtin NON passano dalle variabili
// CSS per certi colori -- li hardcodano direttamente nelle regole (es. body.theme-chalk
// { background-color: #22302a }, body.theme-retro #canvas-container { background-color:
// #f5ecd6 }, i pannelli scuri di body.dark-mode #settings-popup, ecc.). Leggere SOLO la
// variabile (--bg/--surface/--text) restituiva quindi il default di :root (tema chiaro),
// diverso da cio' che l'utente VEDE davvero. Per quei 3 casi si legge il colore EFFETTIVO
// renderizzato dall'elemento giusto (getComputedStyle su background-color/color): con un
// tema che usa le variabili i due valori coincidono, con un tema che hardcoda vince il
// valore reale a schermo -- coerenza garantita per costruzione. _rgbToHex converte il
// formato "rgb(r, g, b)" restituito da queste proprieta' non-custom.
function _effectiveThemeValue(varName) {
  try {
    if (varName === '--bg') {
      const cc = document.getElementById('canvas-container');
      const v = cc ? getComputedStyle(cc).backgroundColor : '';
      if (v && v !== 'rgba(0, 0, 0, 0)' && v !== 'transparent') return v;
      return getComputedStyle(document.body).backgroundColor || _activeVarValue(varName);
    }
    if (varName === '--surface') {
      const sp = document.getElementById('settings-popup');
      const v = sp ? getComputedStyle(sp).backgroundColor : '';
      if (v && v !== 'rgba(0, 0, 0, 0)' && v !== 'transparent') return v;
    }
    if (varName === '--text') {
      const v = getComputedStyle(document.body).color;
      if (v) return v;
    }
  } catch (e) { /* fallback sotto */ }
  return _activeVarValue(varName);
}
function openThemeEditor() {
  ...
  THEME_EDITABLE.forEach(function (grp) {
    ...
    grp.vars.forEach(function (v) {
      // WP-M: _effectiveThemeValue (non _activeVarValue) -- vedi commento sopra: per
      // --bg/--surface/--text i temi builtin possono hardcodare il colore fuori dalle var.
      const cur = _draftColors[v[0]] || _rgbToHex(_effectiveThemeValue(v[0]));
...
// WP-M (Ismail 2026-07-20): pulsante "Imposta" -- applica i colori del draft SENZA salvarli
// come tema (niente voce nel menu, niente download, niente localStorage): i colori restano
// attivi (setDraftColor li ha gia' scritti live su documentElement) finche' non si cambia
// tema o si ricarica la pagina. Chiude solo il popup, SENZA clearCustomVars (differenza
// chiave rispetto a cancelThemeEditor). Riaprendo l'editor si ritrovano gli stessi colori
// (openThemeEditor legge i valori correnti effettivi).
function applyThemeEditorDraft() {
  // Assicura che TUTTI i valori del draft (anche quelli mai toccati, gia' uguali al tema
  // attivo) siano applicati -- di norma e' un no-op per i non modificati.
  try {
    const root = document.documentElement.style;
    Object.keys(_draftColors).forEach(function (k) {
      if (/^--[\w-]+$/.test(k) && _bfIsValidColorVar(_draftColors[k])) root.setProperty(k, _draftColors[k]);
    });
  } catch (e) {}
  if (typeof resizeCanvas === 'function') resizeCanvas();
  else if (typeof draw === 'function' && typeof nodi !== 'undefined') draw(nodi);
...
if (typeof window !== 'undefined') window.applyThemeEditorDraft = applyThemeEditorDraft;
function saveNewTheme() {
  ...
  // WP-M: _effectiveThemeValue, come openThemeEditor (temi builtin con colori hardcoded).
  _allEditableVars().forEach(function (k) { colors[k] = _draftColors[k] || _rgbToHex(_effectiveThemeValue(k)); });
